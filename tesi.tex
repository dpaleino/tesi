\documentclass[a4paper,12pt,openany,titlepage,oneside,BCOR=15mm]{scrbook}
%\usepackage{layaureo}
\usepackage{fontspec}
\usepackage[italian]{babel}
\usepackage{appendix}
\usepackage{ltxtable}
\usepackage{graphicx}
\usepackage{wrapfig}
\usepackage[font=footnotesize,format=plain,labelfont={sf,bf}]{caption}
\usepackage[doublespacing]{setspace}
\usepackage{subfig}
\usepackage{pstricks}
\usepackage{sidecap}
%\usepackage[style=verbose-inote,dashed=false,pageref=true]{biblatex}
\usepackage[style=authortitle-comp,dashed=false]{biblatex}
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{multirow}
\usepackage{makeidx}
\usepackage[italian]{varioref}
\usepackage{hyperref}
\usepackage{remreset}
\usepackage{amstext,amssymb,amsmath}
\usepackage{pdflscape}
\usepackage{dcolumn}
\usepackage{environ}
\usepackage{guit}
\usepackage[signatures]{frontespizio}
\usepackage{eepic,epic,eepicemu}
\usepackage{latexsym}
\usepackage{color}
\usepackage{colortbl}

\addbibresource{biblio.bib}
\renewbibmacro{in:}{}
%\bibliographystyle{natbib_ita}

\addto\captionsitalian{\renewcommand{\figurename}{Fig.}}

\makeindex

\title{Valutazione di analisi cefalometriche tradizionali vs. proporzionali: effetti sulla diagnosi}
\author{}
\date{}
\hypersetup{
  colorlinks=true,
  linkcolor=black,
  citecolor=black,
  urlcolor=black,
  frenchlinks=true,
  pdftitle={Valutazione di analisi cefalometriche tradizionali vs. proporzionali: effetti sulla diagnosi},
  pdfauthor={David Paleino},
  pdfsubject={Cephalometric analysis}
}

\renewcommand{\appendixtocname}{Appendici}
\renewcommand{\appendixpagename}{Appendici}

\makeatletter
\newcommand{\angolo}[1]{$\widehat{#1}$}
\newcommand{\punto}[1]{\textit{#1}}
\newcommand{\piano}[2]{\punto{#1}-\punto{#2}}

\newenvironment{centeredcaption}[1]
  {\def\table@caption{#1}%
   \Collect@Body\measure@table}
  {\begin{minipage}{\table@width}\centering
   \expandafter\caption\expandafter{\table@caption}
   \box0\end{minipage}}
\newenvironment{centeredcaptionlabel}[2]
  {\def\table@caption{#1}%
   \def\table@label{#2}%
   \Collect@Body\measure@table}
  {\begin{minipage}{\table@width}\centering
   \expandafter\caption\expandafter{\table@caption}%
   \expandafter\label\expandafter{\table@label}%
   \box0\end{minipage}}
\def\measure@table#1{\sbox\z@{#1}\edef\table@width{\the\wd\z@}}

\newcommand{\paziente}[4]{
\section*{Paziente #1: #2, #3, #4aa.}
\label{paz:#2}
\null\vspace*{\stretch{1}}
\begin{center}
\fbox{\includegraphics[keepaspectratio,width=.95\columnwidth,height=.7\textheight]{./pazienti/rx/#2.jpg}}
\end{center}
\null\vspace*{\stretch{1}}
\clearpage
\begin{figure}[p]
\centering
\fbox{\includegraphics[keepaspectratio,width=.85\columnwidth,height=.85\textheight]{./pazienti/tracciati/#2E.pdf}}
\caption{Analisi delle controparti di Enlow}
\label{fig:#2E}
\end{figure}
\clearpage
\begin{figure}[p]
\centering
\fbox{\includegraphics[keepaspectratio,width=.85\columnwidth,height=.85\textheight]{./pazienti/tracciati/#2G.pdf}}
\caption{Analisi di Giannì e indice Wits}
\label{fig:#2G}
\end{figure}
\clearpage
\begin{figure}[p]
\centering
\fbox{\includegraphics[keepaspectratio,width=.85\columnwidth,height=.85\textheight]{./pazienti/tracciati/#2R.pdf}}
\caption{Analisi antero-posteriore di Ricketts}
\label{fig:#2R}
\end{figure}
\clearpage
\begin{figure}[p]
\centering
\fbox{\includegraphics[keepaspectratio,width=.85\columnwidth,height=.85\textheight]{./pazienti/tracciati/#2C.pdf}}
\caption{Analisi di Coben}
\label{fig:#2C}
\end{figure}
\clearpage
}
\makeatother

% interlinea
%\linespread{2.5}
\setstretch{2}

% package remreset: don't reset footnote counter at each chapter
\makeatletter\@removefromreset{footnote}{chapter}\makeatother

\begin{document}
\begin{frontespizio}
\Universita{Palermo}
\Logo[3.5cm]{images/unipa}
\Facolta{Medicina e Chirurgia}
\Corso{Odontoiatria e Protesi Dentaria}
\Annoaccademico{2010--2011}
\Titolo{Valutazione di analisi cefalometriche\\tradizionali vs. proporzionali:\\effetti sulla diagnosi}
\Candidato[0506251]{David Paleino}
\Relatore{Ch.mo Prof. Pietro Messina}
\Correlatore{Dott. Maurizio Lotti}
\NCorrelatore {Co-relatore}{Co-relatori}
\end{frontespizio}
\cleardoubleemptypage
\frontmatter
\tableofcontents
\mainmatter
\pagestyle{plain}
\include{chapters/introduzione}
\include{chapters/storia_della_cefalometria}
\include{chapters/crescita_cranio_facciale}
\part*{Analisi convenzionali}
\include{chapters/downs}
\include{chapters/steiner}
\include{chapters/ricketts}
\part*{Analisi proporzionali}
\include{chapters/enlow}
\include{chapters/coben}
\include{chapters/delaire}
\include{chapters/sassouni}
\include{chapters/materiali_metodi}
\include{chapters/discussione}
\include{chapters/conclusioni}

\appendix
\appendixpage

\include{appendices/pazienti}
\include{appendices/misurazioni}
\include{appendices/repere}
\cleardoublepage
\include{appendices/ringraziamenti}

\normalsize
\printbibliography

\end{document}
